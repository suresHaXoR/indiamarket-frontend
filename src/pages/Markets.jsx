...
function MarketCard({ market, onPredict, prevPrice }) {
  ...
  return (
    <div style={{background:flash==='up'?'rgba(15,183,96,0.04)':flash==='dn'?'rgba(255,64,64,0.04)':'#181816',border:`1px solid ${flash==='up'?'rgba(15,183,96,0.4)':flash==='dn'?'rgba(255,64,64,0.4)':'rgba(245,240,232,0.07)'}`,borderRadius:'3px',overflow:'hidden',transition:'all .3s',marginBottom:'10px'}}>
      ...
      {res && <div style={{padding:'0 18px 12px',fontSize:'13px',color:market.outcome==='yes'?'#0FB760':'#FF4040'}}>Outcome: <strong>{market.outcome?.toUpperCase()} WON ✓</strong></div>}
      {!res && (
        <div style={{padding:'0 18px 12px',display:'flex',alignItems:'center',gap:'10px'}}>
          <div style={{minWidth:'54px',textAlign:'center'}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'28px',color:'#0FB760',lineHeight:1}}>{market.yesPrice}¢</div>
          ...
          <div style={{flex:1,height:'10px',background:'rgba(245,240,232,0.05)',borderRadius:'2px',overflow:'hidden',position:'relative'}}>
            <div style={{position:'absolute',left:0,top:0,bottom:0,width:market.yesPrice+'%',background:'linear-gradient(90deg,#07924A,#0FB760)',borderRadius:'2px',transition:'width .4s'}}/>
            <div style={{position:'absolute',right:0,top:0,bottom:0,width:market.noPrice+'%',background:'linear-gradient(270deg,#CC2020,#FF4040)',borderRadius:'2px',transition:'width .4s'}}/>
          </div>
          <div style={{minWidth:'54px',textAlign:'center'}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'28px',color:'#FF4040',lineHeight:1}}>{market.noPrice}¢</div>
      ...
      {!res && (
        <div style={{padding:'0 18px 16px',display:'flex',gap:'9px'}}>
          {[['yes','Predict YES @ '+market.yesPrice+'¢','rgba(15,183,96,0.14)','#0FB760','rgba(15,183,96,0.28)'],
            ['no','Predict NO @ '+market.noPrice+'¢','rgba(255,64,64,0.09)','#FF4040','rgba(255,64,64,0.22)']].map(([side,label,bg,color,border]) => (
            <button key={side} onClick={() => onPredict(market.id, side)} style={{flex:1,padding:'11px',fontFamily:"'Rajdhani',sans-serif",fontSize:'13px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',border:`1px solid ${border}`,background:bg,color,borderRadius:'2px',cursor:'pointer',transition:'all .15s'}}>
              {label}
            </button>
          ))}
        </div>
...
export default function Markets() {
  const { markets, balance, activity, placeBet, resolveMarket } = useStore();
  ...
  useEffect(() => {
    setPrev(p => { const n={...p}; markets.forEach(m=>{if(!n[m.id])n[m.id]=m.yesPrice;}); return n; });
    const t = setInterval(() => {
      setPrev(p => { const n={...p}; markets.forEach(m=>{n[m.id]=m.yesPrice;}); return n; });
    }, 2000);
  ...
  function doPredict() {
    ...
    if (ok) {
      setToast({ title:'✅ Prediction placed!', sub:`₹${amt} on ${slip.side.toUpperCase()} — "${market?.title?.slice(0,30)}..."` });
      setSlip(null);
    }
  }
    ...
    <div style={{display:'flex',height:'100%',overflow:'hidden'}}>
      <div style={{flex:1,overflowY:'auto',padding:'20px 22px',borderRight:'1px solid rgba(245,240,232,0.07)'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'18px',position:'sticky',top:0,background:'#0C0B09',paddingBottom:'14px',zIndex:5}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'22px',letterSpacing:'1px'}}>📊 Live Predictions</div>
        ...
        {filtered.map(m => <MarketCard key={m.id} market={m} onPredict={openSlip} prevPrice={prevPrices[m.id]} />)}
      </div>
      ...
      <div style={{width:'340px',flexShrink:0,overflowY:'auto',padding:'18px 16px',display:'flex',flexDirection:'column',gap:'16px'}}>
        ...
        <div style={{background:'#181816',border:'1px solid rgba(245,240,232,0.07)',borderRadius:'3px',overflow:'hidden'}}>
          ...
          <div style={{maxHeight:'120px',overflowY:'auto'}}>
            {activity.length === 0 && <div style={{padding:'16px',textAlign:'center',fontSize:'13px',color:'rgba(245,240,232,0.3)',fontStyle:'italic'}}>Waiting for predictions...</div>}
            {activity.map((a,i) => (
        ...
        <div style={{background:'#181816',border:'1px solid rgba(245,240,232,0.07)',borderRadius:'3px',overflow:'hidden'}}>
          <div style={{padding:'13px 16px',borderBottom:'1px solid rgba(245,240,232,0.07)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <span style={{fontFamily:"'Rajdhani',sans-serif",fontSize:'15px',fontWeight:700,letterSpacing:'1px',textTransform:'uppercase'}}>🎯 Prediction Slip</span>
          ...
          <div style={{padding:'16px'}}>
            {!slip ? (
              <div style={{textAlign:'center',padding:'24px 0',color:'rgba(245,240,232,0.3)',fontSize:'14px'}}>
                ...
                Click <strong style={{color:'#F0EBE0'}}>Predict YES / NO</strong> to start
              </div>
            ) : (
              <>
                ...
                {amt > 0 && (
                  <div style={{background:'rgba(245,240,232,0.03)',border:'1px solid rgba(245,240,232,0.07)',borderRadius:'3px',padding:'12px 14px',marginBottom:'14px'}}>
                    {[['Potential Reward','₹'+payout.toLocaleString('en-IN'),false],['Reward if correct',(profit>=0?'+':'')+'₹'+Math.abs(profit).toLocaleString('en-IN'),true],['ROI',Math.round(profit/amt*100)+'%',false]].map(([l,v,g]) => (
                ...
                <button onClick={doPredict} disabled={amt<10||amt*100>balance} style={{width:'100%',padding:'15px',fontFamily:"'Rajdhani',sans-serif",fontSize:'16px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',border:'none',borderRadius:'3px',cursor:amt<10||amt*100>balance?'not-allowed':'pointer',background:slip.side==='yes'?'linear-gradient(135deg,#07924A,#0FB760)':'linear-gradient(135deg,#CC2020,#FF4040)',color:'#fff',opacity:amt<10||amt*100>balance?.5:1,marginBottom:'10px',transition:'all .15s'}}>
                  Predict {slip.side.toUpperCase()} @ {price}¢
                </button>
